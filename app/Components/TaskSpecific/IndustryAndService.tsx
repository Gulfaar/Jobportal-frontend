...
const IndustriesAndServices = () => {
  const services = [
    ...
    },
    {
      ...
      icon: <Warehouse size={24} className="text-blue-500" />,
    },
    {
  ...
  return (
    <section className="py-16 px-6 flex justify-center w-full mx-auto bg-[#ECF4F3] relative overflow-hidden">
      <div className="absolute inset-0 flex justify-center items-center">
        <img src="/bgCircle.svg" alt="Background Effect" className="w-[50%] h-auto" />
      </div>
      <div className="max-w-6xl w-full relative z-10">
        <h2 className="text-center text-2xl font-bold text-[#1E3A56] mb-12">INDUSTRIES AND SERVICES WE COVER</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {services.map((service, index) => (
            <div key={index} className="relative bg-transparent p-6 rounded-lg shadow-md flex items-start gap-4">
              <div className="bg-blue-100 p-3 rounded-full shadow-lg flex items-center justify-center">{service.icon}</div>
              <div>
