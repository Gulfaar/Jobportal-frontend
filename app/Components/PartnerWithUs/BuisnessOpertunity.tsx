const BusinessOpportunities = () => {
    ...
    return (
      <section className="bg-[#2E5F5C] py-24 px-16 flex justify-center w-[80%] mx-auto rounded-lg">
        <div className="max-w-5xl w-full flex flex-col space-y-24">
          {opportunities.map((item, index) => (
            <div key={index} className={`flex items-center gap-20 ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
              {/* Text Content */}
              ...
              {/* Image Placeholder */}
              <div className="w-60 h-60 bg-white rounded-lg flex-shrink-0"></div>
            </div>
          ))}
        </div>
      </section>
