...
const Joblisting = () => {
    ...
    const fetchAllJobs = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          console.error("Failed to fetch all jobs:", response.message);
      ...
      if (searchQuery) {
        ...
        try {
          ...
          if (response.success) {
            ...
            console.error("Failed to search jobs:", response.message);
  ...
  const getLogoUrl = (job: Job) => {
    ...
    return job.company?.logo ? `${baseUrl}${job.company.logo}` : "/default-logo.png";
  };
          ...
          <div className="hidden md:block lg:block w-full md:w-1/6 space-y-6 bg-[#EBF5F4] p-4 rounded-lg text-black">
            ...
            <div>
              ...
              <div className="space-y-2">
                {["IT & Technology", "Financial Services", "Telecommunications", "Marketing", "Education"].map(
                  (category) => (
          ...
          <div className="w-full md:w-3/4">
            <div className="flex justify-between items-center mb-6">
              <p className="text-gray-600">
                Showing {indexOfFirstJob + 1}-{Math.min(indexOfLastJob, totalJobs)} of {totalJobs} results
            ...
            {loading ? (
              <p className="text-gray-600 col-span-full text-center">Loading jobs...</p>
            ) : error ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-7 justify-items-center">
                {Array.isArray(currentJobs) && currentJobs.length > 0 ? (
                  currentJobs.map((job) => (
                    <div
                      key={job._id}
                      className="bg-white rounded-lg shadow-md w-full p-7 border relative"
                    >
                      ...
                      <div className="flex gap-3">
                        <button className="flex-1 bg-[#2E5F5C] border border-[#2E5F5C] text-white px-3 py-1.5 rounded text-sm hover:bg-[#234847] transition-colors">
                          Apply now
                        </button>
